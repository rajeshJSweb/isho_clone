             ...
             <div className='lg:py-20 pb-20 bg-[#3BC4F2]'>
                <div className='px-5 sm:px-9 md:mx-[76px] md:px-[30px]'>
                   <div className='flex flex-col lg:flex-row lg:justify-center items-center h-full lg:gap-0 w-full'>
                        ...
                        <h1 className='text-lg mb-1 text-center'>You need to varify your Email!</h1>
                            <p className={`${user&&(user?.message || isError)? 'mb-5 text-base text-center':'mb-10 text-base text-center'}`}>Please enter your valid email and password</p>
                            {user?.message || isError? <div className='flex border-2 border-pink-500 p-2 justify-evenly items-center mb-5 rounded-sm mx-5'>
                                ...
                                <form onSubmit={handleCreateUser} className='w-full flex flex-col items-center gap-3'>
                                    ...
                                    <input className='bg-[#3BC4F2] text-[10px] cursor-pointer lg:text-[14px] my-5 px-5 py-3 rounded-full w-full font-semibold text-white' type="submit" value="CREATE AN ACCOUNT" />
                                </form>
                                <p className='text-gray-500 text-center'>Already have an account? <Link to='/login' className='text-[#3BC4F2] cursor-pointer'>Sign In</Link></p>
                            </div>
                        </div>
                   </div>
                </div>
