    ...
    <div className='w-full'>
      <div className='md:mx-[64px] px-[20px] md:px-[30px] md:flex mt-10 md:border-none border-t-[1px] border-gray-200 gap-10 mb-[100px]'>
        <div className='md:w-6/12 flex justify-center items-center relative pt-2'>
          {product && <img className='w-[200px] transition-transform transform hover:scale-110' src={product.image} alt={product.title} />}
        </div>
          ...
          <div>
            <div className='md:flex items-center gap-5'>
              ...
              <div className='flex w-full overflow-visible justify-center items-center gap-5'>
                <button onClick={() => handleAddToCart(product)} className='px-10 border-[1px] border-pink-600 rounded-full py-4 font-bold hover:bg-slate-100'>
                  ADD CART
                </button>
                <button onClick={() => handleBuyNow(product)} className='px-10 border-[1px] border-pink-600 rounded-full py-4 bg-pink-500 text-white font-bold hover:bg-pink-400'>
                  BUY NOW
                </button>
              </div>
            </div>
            ...
            <div className='mt-[40px] flex flex-col gap-5'>
              ...
              <div className='flex justify-between items-center'>
                <div className='md:w-8/12 w-full pb-[28px]'>
                  ...
                  autoplay={{ delay: 3000 }}
                  >
                    <SwiperSlide className='w-[40px] h-[30px] md:w-[60px] rounded-md md:h-[50px] border-2 border-green-700'>1</SwiperSlide>
            ...
            <div className='md:mt-[50px] mb-[40px] md:mb-0 flex flex-col gap-5'>
              <div className='flex gap-7 md:gap-14 text-[16px] font-semibold'>
                <button onClick={() => filterDetails('description')} className={`underline cursor-pointer ${selectedDetail === 'description' ? 'text-pink-500' : ''}`}>Description</button>
                <button onClick={() => filterDetails('materials')} className={`underline cursor-pointer ${selectedDetail === 'materials' ? 'text-pink-500' : ''}`}>Materials</button>
                <button onClick={() => filterDetails('dimension')} className={`underline cursor-pointer ${selectedDetail === 'dimension' ? 'text-pink-500' : ''}`}>Dimension</button>
              </div>
              <div className=''>
                {selectedDetail === 'description' && <p className=''>{product.description}</p>}
                {selectedDetail === 'materials' && <p className=''>{product.materials}</p>}
                {selectedDetail === 'dimension' && <p className=''>{product.dimension}</p>}
              </div>
            </div>
          </div>
        </div>
